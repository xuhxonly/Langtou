...
public interface ContentMapper extends BaseMapper<Content> {
    ...
    @Select("SELECT * FROM content WHERE user_id = #{userId} AND deleted = 0 ORDER BY create_time DESC")
    ...
    @Update("UPDATE content SET collect_count = collect_count + 1 WHERE id = #{id}")
    int incrementCollectCount(@Param("id") Long id);

    @Update("UPDATE content SET collect_count = collect_count - 1 WHERE id = #{id} AND collect_count > 0")
