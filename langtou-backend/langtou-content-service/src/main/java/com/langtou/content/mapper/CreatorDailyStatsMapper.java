...
public interface CreatorDailyStatsMapper extends BaseMapper<CreatorDailyStats> {
    ...
    @Select("SELECT * FROM creator_daily_stats WHERE creator_id = #{creatorId} ORDER BY date DESC")
    List<CreatorDailyStats> selectByCreatorId(@Param("creatorId") Long creatorId);
