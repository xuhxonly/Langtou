...
Value:   Hash { user_id, username, device_type, login_at }
...
Value:   Hash { id, username, nickname, avatar_url, bio, follower_count, following_count, note_count, liked_count }
TTL:     1小时 + 随机300秒
...
Value:   Hash { id, user_id, title, content, images, video_url, cover_url, location,
               like_count, comment_count, collect_count, share_count, view_count, status, created_at }
TTL:     2小时 + 随机600秒
...
Value:   Hash { follower_count, following_count, note_count, liked_count, unread_notify }
TTL:     永不过期（异步续期）
...
说明:    未读通知分类计数（type与notification.type枚举一致）
```

### 10.3 最近消息列表（会话列表）
```
Key:     lt:message:recent:{user_id}
Value:   SortedSet { score: last_msg_at, member: peer_user_id }
TTL:     7天
