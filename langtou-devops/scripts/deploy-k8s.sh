...
ENVIRONMENT="${1:-dev}"
IMAGE_TAG="${2:-latest}"
DOCKER_REGISTRY="${DOCKER_REGISTRY:-registry.langtou.local}"
DOCKER_NAMESPACE="${DOCKER_NAMESPACE:-langtou}"
...
deploy_services() {
    ...
    for service in "${services[@]}"; do
        ...
        sed "s|image: ${DOCKER_REGISTRY}/${DOCKER_NAMESPACE}/${service}:latest|image: ${DOCKER_REGISTRY}/${DOCKER_NAMESPACE}/${service}:${IMAGE_TAG}|g" \
...
full_deploy() {
    ...
    log_info "镜像仓库: ${DOCKER_REGISTRY}/${DOCKER_NAMESPACE}"
    log_info "========================================"
...
rolling_update() {
    ...
    kubectl set image deployment/$service \
        $service=${DOCKER_REGISTRY}/${DOCKER_NAMESPACE}/$service:$tag \
        -n langtou
